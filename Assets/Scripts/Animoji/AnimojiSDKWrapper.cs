...
namespace MetaVerse.Animoji
{
    ...
    {
        public const int c_unityExpressionSize = 67;
        public const int c_nativeExpressionSize = 59;

        // input 59 float data
        public int getUnityExpressionFromArray(float[] data, float[] unityExpression)
        {
            if (data == null || data.Length != c_nativeExpressionSize || unityExpression.Length != c_unityExpressionSize)
                return -1;
            process59Expression(data, unityExpression);
            return 0;
        }

        // 59 to 67 unity expression
        private void process59Expression(float[] expression_59, float[] unityExpression)
        {
            ...
            bool use_tongue = false)
        {
            ...
            process59Expression(expression_59, unityExpression);
            return result;
        }
