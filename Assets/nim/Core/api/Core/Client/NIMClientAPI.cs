...
namespace NIM
{
    ...
    public class ClientAPI
    {
        ...
        private static void CleanupSystemUtil()
        {
#if UNITY_ANDROID
            try
            {
                using (var clsSystemUtil = new UnityEngine.AndroidJavaClass("com.netease.nimlib.NIMSDK"))
                {
                    if (clsSystemUtil != null)
                    {
                        NimUtility.Log.Info("com.netease.nimlib.NIMSDK found");
						Boolean init = clsSystemUtil.CallStatic<Boolean>("cleanup");
                        NimUtility.Log.Info("boolean:" + init);
                    }
                }
            }
            catch (System.Exception e)
